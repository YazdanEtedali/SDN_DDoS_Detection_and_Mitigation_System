...
class MyController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.model = self.load_model('/home/yazdan/Downloads/prj/catboost_model.pkl')
    ...
    def process_packet(self, pkt):
        ...
        if tcp_pkt:
            ...
            service = self.service_ports.get(dst_port, "http")
            self.logger.info(f"TCP packet: src_ip={src_ip}, dst_ip={dst_ip}, dst_port={dst_port}, service={service}")
        elif udp_pkt:
            dst_port = udp_pkt.dst_port
            service = self.service_ports.get(dst_port, "udp")
            self.logger.info(f"UDP packet: src_ip={src_ip}, dst_ip={dst_ip}, dst_port={dst_port}, service={service}")
        elif icmp_pkt:
            service = "icmp"
            self.logger.info(f"ICMP packet: src_ip={src_ip}, dst_ip={dst_ip}, service={service}")

        # Process the features
        self.process_features(src_ip, dst_ip, dst_port, service, tcp_pkt, udp_pkt, icmp_pkt)
